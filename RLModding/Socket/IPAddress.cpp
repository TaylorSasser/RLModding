...
IPAddress::IPAddress(uint8_t octet1, uint8_t octet2, uint8_t octet3, uint8_t octet4) noexcept : m_Valid(true) {
	m_Address = (octet1 << 24) | (octet2 << 16) | (octet3 << 8) | octet4;
}
...
void IPAddress::SetOctet(uint8_t num, uint8_t value) {
	...
	if (num == 0 || num > 4) throw std::invalid_argument("Invalid argument in IPAddress:SetOctet.");
